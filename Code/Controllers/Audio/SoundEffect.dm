...
/SoundEffect/New(var/SoundDef/Def, var/atom/PositionRef, var/Channel)
	...
	var/SoundFile
	Audio.AudioCache["Sounds\\" + Definition.SoundFile] >> SoundFile

	Sound = sound(SoundFile, channel = Channel)
	Sound.environment = Definition.Environment || Audio.Environment
...
/SoundEffect/proc/TickFade()
	...
	if (!FadeDone)
		...
		Sound.volume = ((SFXVolume / MaxVolume) * Volume * Definition.VolumeMultiplier)
...
/SoundEffect/proc/Play(var/Emitter)
	...
	if (Audio.IsChannelAllocated(Sound.channel) && !(Definition.SoundType & SoundTypeAmbience))
		Audio.PreserveChannelAllocation(src)
	else if (!Audio.IsChannelAllocated(Sound.channel))
		Sound.channel = Audio.AllocateChannel(src)
